package tv.mineinthebox.essentials.commands;

import java.util.HashMap;

import org.bukkit.Bukkit;
import org.bukkit.ChatColor;
import org.bukkit.command.Command;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;

import tv.mineinthebox.essentials.xEssentials;
import tv.mineinthebox.essentials.permissions.consolePermission;
import tv.mineinthebox.essentials.permissions.playerPermission;

public class cmdtpahere {

	private static HashMap<String, String> players = new HashMap<String, String>();

	public static boolean execute(final CommandSender sender, Command cmd, String[] args) {
		if(cmd.getName().equalsIgnoreCase("tpahere")) {
			if(sender instanceof Player) {
				if(sender.hasPermission("xEssentials.command.tpahere")) {
					if(args.length == 0) {
						sender.sendMessage(ChatColor.GOLD + ".oO___[Tpahere help]___Oo.");
						sender.sendMessage(ChatColor.DARK_GRAY + "Default: " + ChatColor.GRAY + "/tpahere <player> " + ChatColor.WHITE + ": sents a tpa request to a player");
						sender.sendMessage(ChatColor.DARK_GRAY + "Default: " + ChatColor.GRAY + "/tpahere accept " + ChatColor.WHITE + ": accept a request");
						sender.sendMessage(ChatColor.DARK_GRAY + "Default: " + ChatColor.GRAY + "/tpahere deny " + ChatColor.WHITE + ": deny a request");
					} else if(args.length == 1) {
						if(args[0].equalsIgnoreCase("help")) {
							sender.sendMessage(ChatColor.GOLD + ".oO___[Tpahere help]___Oo.");
							sender.sendMessage(ChatColor.DARK_GRAY + "Default: " + ChatColor.GRAY + "/tpahere <player> " + ChatColor.WHITE + ": sents a tpa request to a player");
							sender.sendMessage(ChatColor.DARK_GRAY + "Default: " + ChatColor.GRAY + "/tpahere accept " + ChatColor.WHITE + ": accept a request");
							sender.sendMessage(ChatColor.DARK_GRAY + "Default: " + ChatColor.GRAY + "/tpahere deny " + ChatColor.WHITE + ": deny a request");
						} else if(args[0].equalsIgnoreCase("accept")) {
							if(players.containsKey(sender.getName())) {
								Player accepter = (Player) sender;
								Player inviter = Bukkit.getPlayer(players.get(accepter.getName()));
								if(accepter instanceof Player && inviter instanceof Player) {
									accepter.sendMessage(ChatColor.GREEN + "you accepted " + inviter.getName() + " his request!");
									inviter.sendMessage(ChatColor.GREEN + accepter.getName() + " has accepted your request!");
									cmdteleport.teleport(accepter, inviter);
									players.remove(accepter.getName());
								} else {
									sender.sendMessage(ChatColor.RED + "it seems that " + players.get(accepter.getName()) + " is no longer online!");
									players.remove(accepter.getName());
								}
							} else {
								sender.sendMessage(ChatColor.RED + "you don't have any tpahere requests open!");
							}
						} else if(args[0].equalsIgnoreCase("deny")) {
							if(players.containsKey(sender.getName())) {
								Player denier = (Player) sender;
								Player inviter = Bukkit.getPlayer(players.get(denier.getName()));
								if(denier instanceof Player && inviter instanceof Player) {
									denier.sendMessage(ChatColor.GREEN + "you denied " + inviter.getName() + " his request!");
									inviter.sendMessage(ChatColor.GREEN + denier.getName() + " has denied your request!");
									players.remove(denier.getName());
								} else {
									sender.sendMessage(ChatColor.RED + "it seems that " + players.get(denier.getName()) + " is no longer online!");
									players.remove(denier.getName());
								}
							} else {
								sender.sendMessage(ChatColor.RED + "you don't have any tpahere requests open!");
							}
						} else {
							final Player victem = Bukkit.getPlayer(args[0]);
							if(victem instanceof Player) {
								if(!players.containsKey(victem.getName())) {
									victem.sendMessage(ChatColor.GREEN + sender.getName() + " wants to teleport you to his location\n/tpahere accept\n/tpahere deny");
									sender.sendMessage(ChatColor.GREEN + "tpahere request successfully sent to " + args[0]);
									players.put(victem.getName(), sender.getName());
									final String usender = sender.getName();
									final String victemname = victem.getName();
									Bukkit.getScheduler().scheduleSyncDelayedTask(xEssentials.getPlugin(), new Runnable() {

										@Override
										public void run() {
											if(players.containsKey(victemname)) {
												if(victem instanceof Player) {
													sender.sendMessage(ChatColor.RED + "tpahere request from " + usender + " has been canceled");
												}
												if(sender instanceof Player) {
													sender.sendMessage(ChatColor.RED + "your tpahere request has been overtime");
												}
												players.remove(victemname);
											}
										}
										
									}, 1000);
								} else {
									sender.sendMessage(ChatColor.RED + args[0] + " has to much open requests please wait some time");
								}
							} else {
								sender.sendMessage(ChatColor.RED + args[0] + " is not online!");
							}
						}
					}
				} else {
					playerPermission.getPermissionError(sender, cmd, args);
				}
			} else {
				consolePermission.getConsoleMessage(sender);
			}
		}
		return false;
	}

}
